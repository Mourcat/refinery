...
class Bearing(models.Model):
    ...
    image = models.ImageField("Изображение", upload_to="source/bearings/", help_text='изображение', null=True, blank=True)
...
class MechSeal(models.Model):
    ...
    slug = models.SlugField("URL", max_length=50, null=False, unique=True, editable=True, help_text='URL')
    image = models.ImageField("Изображение", upload_to="source/mech_seals/", help_text='изображение', null=True, blank=True)
    ...
    class Meta:
        ...
        indexes = [
            models.Index(fields=['manufacturer', 'assembly_code']),
        ]
        
    def __str__(self):
        return f'{self.manufacturer} {self.assembly_code}'